...
function BetterInbox:OnInitialize()
	self.db = LibStub("AceDB-3.0"):New("BetterInboxDB", defaults, "Default")
	db = self.db.profile
end
...
function BetterInbox:TakeAll( first )
	...
	if not checked[mIndex] and ( db.openAllValue == "sall" or db.openAllValue == "sitems" or db.openAllValue == "sgold" ) then
		-- skip unchecked mail
		mIndex = mIndex -1
		aIndex = -1
		return self:TakeAll()
	...
	if subject then
		...
		-- valid mail
		if aIndex == 0 or db.openAllValue == "gold" or db.openAllValue == "sgold" then -- all attachments passed, try and get the moneys
			-- take money
			if money > 0 and db.openAllValue ~= "items" and db.openAllValue ~= "sitems" then
...
function BetterInbox:InboxFrameItem_OnEnter(this)
