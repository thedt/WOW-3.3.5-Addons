...
function WhoTaunted:HideLFGTankTaunts(Unit)
	local t,h,d,n = UnitGroupRolesAssigned(Unit);
	if(UnitInParty("player") and t == true and WhoTaunted.db.profile.HideLFGTankTaunts == true) then
		return true;
	else
		return false;
	end
...
function WhoTaunted:HideTankUnit(Unit)
	local myClass = UnitClass(Unit);
	if 	(myClass == "WARRIOR" and GetBonusBarOffset() == 2 or
		myClass == "DRUID" and GetBonusBarOffset() == 3 or
		myClass == "PALADIN" and UnitAura(Unit, GetSpellInfo(25780)) or
...
function WhoTaunted:LFG_DPS_TANK_CHECK(Unit)
	local myClass = UnitClass(Unit);
	local t,h,d,n = UnitGroupRolesAssigned(Unit);
	if 	(t == false and (myClass == "WARRIOR" and GetBonusBarOffset() == 2 or
		myClass == "DRUID" and GetBonusBarOffset() == 3 or
		myClass == "PALADIN" and UnitAura(Unit, GetSpellInfo(25780)) or
		myClass == "DEATHKNIGHT" and GetShapeshiftForm() ~= 0 and GetShapeshiftFormInfo(GetShapeshiftForm()) == "Interface\\Icons\\Spell_Deathknight_FrostPresence")) then
		return true;
	...
	if (Event) and (Name) and (ID) then
		if (WhoTaunted.db.profile.Disabled == false) and (BgDisable == false) and (UnitIsPlayer(Name)) and ((UnitInParty("player")) or (UnitInRaid("player"))) and ((UnitInParty(Name)) or (UnitInRaid(Name))) then
			...
			if (Event == "SPELL_AURA_APPLIED") then
				...
				if (not Target) or (not IsTaunt) or (TauntType ~= TauntTypes.Normal) or (WhoTaunted.db.profile.AnounceTaunts == false) or ((WhoTaunted.db.profile.HideOwnTaunts == true) and (Name == PlayerName)) or WhoTaunted:HideLFGTankTaunts(Name) == true then
					return;
				end
			...
			elseif (Event == "SPELL_CAST_SUCCESS") then
				IsTaunt, TauntType = WhoTaunted:IsTaunt(ID);
				if (not IsTaunt) or (TauntType ~= TauntTypes.AOE) or (WhoTaunted.db.profile.AnounceAOETaunts == false) or ((WhoTaunted.db.profile.HideOwnTaunts == true) and (Name == PlayerName)) or WhoTaunted:HideLFGTankTaunts(Name) == true then
					return;
				end
				OutputType = WhoTaunted:GetOutputType(TauntType);
				local Spell = GetSpellLink(ID);
